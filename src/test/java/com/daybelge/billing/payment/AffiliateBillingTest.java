...
public class AffiliateBillingTest extends PaymentTestBase {
	...
	public void calculatePayableAmountBelow100() {
		...
		Bill bill = BillBuilder.forUser(michael).addItem(groceries).addItem(clothes).setCurrencyConverter(converter)
				.build();
		MoneyAmount payableAmount = bill.calculatePayableAmount();

		assertEquals("Payable amount incorrect.", MoneyAmount.dollar(33), payableAmount);
	}

	@Test
	public void calculatePayableAmountOver100() {

		Bill bill = BillBuilder.forUser(michael).addItem(groceries).addItem(clothes).addItem(books)
				.setCurrencyConverter(converter).build();
		MoneyAmount payableAmount = bill.calculatePayableAmount();

		assertEquals("Payable amount incorrect.", MoneyAmount.dollar(288), payableAmount);
	}
}
